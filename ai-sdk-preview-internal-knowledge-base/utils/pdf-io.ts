...
export class PDFIO {
  ...
  constructor(
    ...
    this.pdfDoc = PDFDocumentFactory.load(
      (options?.isBuffer ? (file as Buffer) : fs.readFileSync(file)) as Uint8Array
    );
    this.imagesInDoc = [];
    this.objectIdx = 0;
  ...
  async extractImages(): Promise<(Uint8Array<ArrayBufferLike> | Buffer)[] | undefined> {
    ...
    if (this.options?.outputDirectory || !this.options?.isBuffer) {
      ...
      for (const img of this.imagesInDoc) {
        if (!img.isAlphaLayer) {
          const imageData =
            (img.type === "jpg" ? img.data : await this.saveImageAsPNG(img)) as Uint8Array;
          fs.writeFileSync(
